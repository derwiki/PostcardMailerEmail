...
RSpec.describe CreatePostcard, type: :service do
  describe '#run' do
    ...
    before do
      ...
      allow(mock_request).to receive(:[]=).with('Content-Type', 'application/json')
      allow(mock_request).to receive(:[]=).with('Accept', 'application/json')
      allow(mock_request).to receive(:[]=).with('Authorization', "Basic #{ENV['DIRECT_MAIL_KEY']}")
    end

    it 'sends a properly formatted request to the Direct Mail API' do
      postcard = described_class.new(from_address, to_address, image_url, message, dryrun: true)
      response = postcard.run

      # Verify HTTP setup
      expect(Net::HTTP).to have_received(:new).with('print.directmailers.com', 443)
      ...
      expect(mock_request).to have_received(:body=) do |json_body|
        ...
        expect(parsed_body['Front']).to include("Brighten someone's day, send a free postcard at postcardmailer.us")
      end
