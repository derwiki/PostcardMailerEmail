...
RSpec.describe SendgridPostHandler do
  ...
  let(:mail_double) do
    double("Mail", deliver_now: true, bcc: nil).tap do |mail|
  ...
  describe "#process" do
    context "when user and address exist" do
      ...
      it "processes the postcard successfully" do
        ...
        expect(EssThree).to have_received(:upload).with(
          "test-uuid.jpg",
          "test-image-data"
        )
    ...
    context "when handling signup request" do
      ...
      it "creates a new user and address" do
        expect { handler.process }.to change(User, :count).by(1).and change(
                Address,
                :count
              ).by(1)
      ...
      context "when user already exists" do
        ...
        it "does not create a new user or address" do
          expect { handler.process }.to change(User, :count).by(0).and change(
                  Address,
                  :count
                ).by(0)
        end
    ...
    context "when handling approve request" do
      let(:unverified_user) do
        create(:user, email: "pending@example.com", verified_at: nil)
      end
      let(:params) do
        {
    ...
    context "when DKIM fails" do
      it "sends an error email and returns early" do
        params_with_failed_dkim =
          valid_params.merge(dkim: "{@example.com : fail}")
