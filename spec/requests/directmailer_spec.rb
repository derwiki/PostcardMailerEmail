...
RSpec.describe "Directmailer Webhooks", type: :request do
  let(:fixture_path) do
    Rails.root.join("test", "fixtures", "directmailer_webhook_payload.json")
  end
  let(:webhook_payload_hash) { JSON.parse(File.read(fixture_path)) }
  let(:print_record_id_from_fixture) do
    webhook_payload_hash.dig("Data", 0, "PrintRecord")
  end
  ...
  describe "POST /webhook" do
    context "when a matching Postcard exists" do
      ...
      before do
        ...
        allow(PostcardLifecycleMailer).to receive_message_chain(
          :status_update,
          :deliver_now
        )
      end
      ...
      it "processes the webhook successfully" do
        matching_postcard =
          Postcard.find_by(print_record_id: print_record_id_from_fixture)
      ...
      it "records the webhook event in directmailers_events" do
        expect {
          post "/webhook", params: webhook_payload, as: :json
        }.to change { postcard.reload.directmailers_events.size }.by(1)
      ...
      it "sends a status update email" do
        expect(PostcardLifecycleMailer).to receive_message_chain(
          :status_update,
          :deliver_now
        )
    ...
    context "when the payload is malformed (e.g., Data is not an array)" do
      let(:malformed_payload) do
        { Event: "NewPrintObject", Object: "Postcard", Data: "not_an_array" }
      end
