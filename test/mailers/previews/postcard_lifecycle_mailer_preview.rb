...
class PostcardLifecycleMailerPreview < ActionMailer::Preview
  ...
  def status_update_error
    postcard =
      find_or_create_sample_postcard(
        "Error",
        "There was a problem processing your postcard"
      )
    PostcardLifecycleMailer.status_update(postcard)
  end
  ...
  def find_or_create_sample_postcard(status = "Mailed", description = nil)
    ...
    user = User.new(id: 9999, email: "sarah.johnson@example.com")
    ...
    postcard =
      Postcard.new(
        ...
        message:
          "Grandma, thinking of you during my vacation in California. The weather is beautiful! Miss you lots.",
        ...
        response_data: {
          ...
          "ActualDeliveryDate" =>
            status == "Delivered" ? Time.current.iso8601 : nil
        },
